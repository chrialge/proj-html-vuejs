<script>
export default {
    name: 'AboutUs',
    data() {

        return {}
    }
}
</script>

<template>
    <section id="about-us">
        <div class="container_fluid d-flex">
            <div class="d-flex top-about-us">
                <div class="left col-6">
                    <img src="/images/business-people-working-together-on-project-and-5FHSKBL.jpg"
                        alt="people working together on project">
                </div>
                <div class="right col-5">
                    <div class="section">
                        <div class="separeted_small"></div>
                        <span class="space-letter">ABOUT US</span>
                    </div>
                    <h1>WE MAKE YOUR STRATEGY WORK</h1>
                    <p class="paragraph">
                        When, while lovely valley teems with vapour around meand eridian sun strikes the upper
                        impernetraable foliage of my trees, and but a thousand unknow plant are noticed when i hear the
                        buzz
                        of the little world among the stalks and grow familiar indescribable forms of the insects and
                        files,
                        the i feel the presence of the almighty
                    </p>
                </div>
            </div>

            <div class="banner d-flex col-11 posi-rela">
                <div class="left_banner">
                    <h1>WE HELP TO DELIVER THE BUSINESS CASE</h1>
                </div>
                <div class="right_banner">
                    <p>
                        When, while lovely valley teems with vapour around meand eridian sun strikes the upper
                        impernetraable foliage of my trees, and but a thousand unknow
                    </p>

                </div>
                <button class="posi-abso">GET STARTED</button>
            </div>
        </div>

    </section>
</template>

<style></style>